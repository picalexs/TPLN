...
TEMPORAL_INPUT = CLUSTERED_PARQUET
...
print(f"Loaded {len(df)} rows from {TEMPORAL_INPUT}")

df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
df["timestamp_source"] = df.get("timestamp_source", pd.Series(index=df.index, dtype="object")).fillna("missing").astype(str).str.lower()
...
for i, cluster_id in enumerate(cluster_ids):
    ...
    rep_title = str(sub_all.iloc[0].get("title", ""))[:200] if len(sub_all) > 0 else ""
